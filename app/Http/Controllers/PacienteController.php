...
class PacienteController extends Controller
{
    ...
    public function store_recepcionista(Request $request)
    {
        try {
            // Validação dos dados de entrada
            $validatedData = $request->validate([
            ...
            return redirect('/painel-recepcionista/gestao-paciente')->with('success', 'Paciente cadastrado com sucesso!');
        } catch (ValidationException $e) {
            // Captura os erros de validação
            //return redirect()->back()->with('error', $e->validator->errors());
            return redirect('/painel-recepcionista/gestao-paciente')->with('error', 'Já existe um paciente com este CPF, RG, email ou número do cartão do plano de saúde. Por favor, verifique os dados e tente novamente.');
        } catch (\Exception $e) {
            // Captura outros tipos de exceção
            return redirect('/painel-recepcionista/gestao-paciente')->with('error', 'Erro ao inserir o paciente. Por favor, tente novamente.');
        }
    }
    ...
    public function update_paciente_recepcionista(Request $request, $id)
    {
        try {
            // Validação dos dados de entrada
            // Validar os dados do formulário
            ...
            // Redirecionar ou retornar resposta de sucesso
            return redirect('/painel-recepcionista/gestao-paciente')->with('success', 'Paciente atualizado com sucesso!');
        } catch (ValidationException $e) {
            // Captura os erros de validação
            //return redirect()->back()->with('error', $e->validator->errors());
            return redirect('/painel-recepcionista/gestao-paciente')->with('error', 'Já existe um paciente com este CPF, RG, email ou número do cartão do plano de saúde. Por favor, verifique os dados e tente novamente.');
            ...
            // Captura outros tipos de exceção
            //return redirect()->back()->with('error', $e->getMessage())->withInput();
            return redirect('/painel-recepcionista/gestao-paciente')->with('error', 'Erro ao alterar o paciente. Por favor, tente novamente mais tarde.');
        }
    }
    ...
    public function search_recepcionista(Request $request)
    {
        try {
            $search = $request->input('search');
            if (!empty($search)) {
